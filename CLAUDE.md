...
- **Banned AI tells (locked 2026-08-07).** Two things never appear in copy: the em dash character, and "honest"/"honestly"/"candidly"/"full transparency" used to vouch for what follows. Applies to body copy, headings, page titles, meta descriptions, alt text, and MDX comments, on every post and every page. Use a comma, colon, parentheses, semicolon, or two sentences in place of an em dash; say the point plainly in place of "honest". Two exemptions, both verbatim quotes of someone else's words: guest reviews in `src/lib/site.ts` and Will's host notes in `src/lib/guidebook.ts`. Enforced by `blog-writer.md` voice principles #6/#7 and `blog-seo-editor.md` checklist #10 (grep, either hit is a FAIL).
