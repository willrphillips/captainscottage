...
**~88% built — and LIVE at https://captainscottageva.com (launched 2026-05-26).**
...
| Phase 5 — Polish + launch (domain, analytics, GSC) | 10% | ~80% | 8.0% |
...
- **RESUME POINTER (start here, 2026-06-02):** Site is LIVE and self-publishing; batch 1 auto-ships weekly from 2026-06-03 (Crabbing) at 12:00 UTC / 08:00 ET. **Post-launch SEO checklist — remaining:** (a) **Bing Webmaster** — Will to import from GSC (in progress); (b) **IndexNow** key (from Bing) → drop at `public/<key>.txt` → scaffold the ping-on-deploy GitHub Action; (c) **PageSpeed Insights** spot check on home + a journal page; (d) **Airbnb listing description** — add the new URL (highest-value backlink, Will's action). **Pending Will (content):** review + approve batch-2's 5 drafts (no `approvedAt` yet → won't publish until approved); answer the cost-to-own / wildlife / DC / Irvington TODO placeholders; trigger the `guest-reply-bootstrap` when ready to seed the guest-reply knowledge base.
