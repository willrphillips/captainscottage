...
2. **Negatives, when honest and necessary, are handled with care:** (a) **one at a time** — never stack jellyfish + tides + bugs (or any multiple downsides) in the same passage; give an honest topic its own calm space; (b) immediately counterweighted with *there is plenty else to do*; (c) at most one quiet *"some things are out of our control"* line — gracious, not anxious; (d) tone stays light and matter-of-fact. Never doom, never a stress list.
